...
func (st *sqlStore) SaveAnswer(a answer) error {
	tx, err := st.db.Begin()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("could not query option rows: %v", err)
	}
	defer close(rows)
	var optionid int
	...
	for rows.Next() {
		...
		if err != nil {
			return fmt.Errorf("could not scan optionid: %v", err)
		}
	...
	if err != nil {
		return fmt.Errorf("could not increment option counter: %v", err)
	}
